...
export class Planner {
  ...
  async generatePlan(prompt: string): Promise<ProjectPlan> {
    ...
    try {
      ...
      planSize = config.get<string>('planSize', 'Normal');
...
Need help? Visit: https://github.com/manasdutta04/layr/issues`;
  ...
  private loadConfig(): LayrConfig {
    ...
    return {
      ...
      groq: {
        apiKey: groqApiKey,
        model: this.aiModel as 'llama-3.3-70b-versatile'
      }
    };
  }
  ...
  private validateFileStructure(items: unknown[]): FileStructureItem[] {
    ...
    return items.map(item => {
      const typedItem = item as { name?: string; type?: string; description?: string; children?: unknown[] };
      return {
  ...
  private validateNextSteps(steps: unknown[]): PlanStep[] {
    ...
    return steps.map((step, index) => {
      const typedStep = step as { id?: string; description?: string; priority?: string; estimatedTime?: string; dependencies?: string[] };
      const priority = typedStep.priority;
      return {
        id: typedStep.id || `step-${index}`,
        description: typedStep.description || 'No description',
        completed: false,
        priority: (priority === 'high' || priority === 'medium' || priority === 'low') ? priority : 'medium',
        estimatedTime: typedStep.estimatedTime || 'Unknown',
        dependencies: Array.isArray(typedStep.dependencies) ? typedStep.dependencies : []
      };
    });
  }

  private renderFileStructure(items: FileStructureItem[], markdown: string[], depth: number): void {
